...
const D = h('endcomment', Z, x),
    ...
    if (O.next == 35) {
      for (
        ...
        !(
          O.next == 10 ||
          O.next < 0 ||
          ((O.next == 37 || O.next == 125) && O.peek(1) == 125)
        );
      )
        O.advance();
      O.acceptToken(V);
  ...
  A = T.deserialize({
    ...
    repeatNodeCount: 11,
    tokenData:
      ")T~RkXY!vYZ!v]^!vpq!vqr#Xrs#duv$Uwx$axy$|yz%R{|%W|}&r}!O&w!O!P'T!Q![&a![!]'e!^!_'j!_!`'r!`!a'j!c!}'z#R#S'z#T#o'z#p#q(s#q#r(x%W;'S'z;'S;:j(m<%lO'z~!{S$}~XY!vYZ!v]^!vpq!v~#[P!_!`#_~#dOa~~#gUOY#dZr#drs#ys;'S#d;'S;=`$O<%lO#d~$OOn~~$RP;=`<%l#d~$XP#q#r$[~$aOw~~$dUOY$aZw$awx#yx;'S$a;'S;=`$v<%lO$a~$yP;=`<%l$a~%ROf~~%WOe~P%ZQ!O!P%a!Q![&aP%dP!Q![%gP%lRoP!Q![%g!g!h%u#X#Y%uP%xR{|&R}!O&R!Q![&XP&UP!Q![&XP&^PoP!Q![&XP&fSoP!O!P%a!Q![&a!g!h%u#X#Y%u~&wOt~~&zRuv$U!O!P%a!Q![&a~'YQ]S!O!P'`!Q![%g~'eOh~~'jOs~~'oPa~!_!`#_~'wPd~!_!`#__(TW^WvQ%QT}!O'z!Q!['z!c!}'z#R#S'z#T#o'z%W;'S'z;'S;:j(m<%lO'z_(pP;=`<%l'z~(xOq~~({P#q#r)O~)TOX~",
    tokenizers: [F, C, D, N, 0, 1, 2, 3],
    topRules: { Template: [0, 7] },
    specialized: [
      { term: 186, get: O => I[O] || -1 },
      { term: 38, get: O => L[O] || -1 },
    ],
    tokenPrec: 0,
  });
...
const m = o(
  ...
  f = o('empty forloop tablerowloop in with as', 'keyword'),
  B = o('first index index0 last length rindex', 'property'),
  H = o(
...
function M(O) {
  var a;
  let { state: e, pos: r } = O,
    $ = S(e).resolveInner(r, -1).enterUnfinishedNodesBefore(r),
    i =
      ((a = $.childBefore(r)) === null || a === void 0 ? void 0 : a.name) ||
      $.name;
...
function J(O = {}) {
  let a = O.filters ? O.filters.concat(m) : m,
    e = O.tags ? O.tags.concat(q) : q,
    r = O.variables ? O.variables.concat(f) : f,
    { properties: $ } = O;
  return i => {
    var n;
    let Q = M(i);
    ...
    return (
      Q.type == 'filter'
        ...
        : Q.type == 'tag'
          ...
          : Q.type == 'expression'
            ? (l = r)
            : (l = K(i.state, Q.target, i, $)),
      l.length
        ? { options: l, from: s, validFor: /^[\w\u00c0-\uffff]*$/ }
        : null
    );
  };
}
const OO = d.inputHandler.of((O, a, e, r) =>
  r != '%' || a != e || O.state.doc.sliceString(a - 1, e + 1) != '{}'
    ? !1
    : (O.dispatch(
        O.state.changeByRange($ => ({
          changes: { from: $.from, to: $.to, insert: '%%' },
          range: y.cursor($.from + 1),
        })),
      ...
      props: [
        ...
        j.add({
          'UnlessDirective ForDirective TablerowDirective CaptureDirective IfDirective CaseDirective RawDirective Comment'(
            ...
            return !a || a.name != 'Tag'
              ? null
              : { from: a.to, to: e.name == 'EndTag' ? e.from : O.to };
          },
        }),
      ],
    }),
...
const aO = w(P.language);
function rO(O = {}) {
  let a = O.base || P,
    e = a.language == P.language ? aO : w(a.language);
  return new _(e, [
    a.support,
