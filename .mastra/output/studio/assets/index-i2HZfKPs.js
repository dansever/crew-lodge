...
const iO = new w({ start: !1, shift: (Q, e) => (e == q ? Q : tO.has(e)) }),
  XO = x({
...
const p = /^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/,
  ...
  rO = Q => {
    ...
    let n =
      e.name == 'VariableName' ||
      (e.to - e.from < 20 && p.test(Q.state.sliceDoc(e.from, e.to)));
...
function TO() {
  let Q = oO.concat($O);
  return new k(r, [
    r.data.of({ autocomplete: h(m, f(Q)) }),
    r.data.of({ autocomplete: rO }),
  ]);
}
export {
  TO as go,
