...
function A(O) {
  var i;
  let { state: t, pos: r } = O,
    e = G(t).resolveInner(r, -1).enterUnfinishedNodesBefore(r),
    P =
      ((i = e.childBefore(r)) === null || i === void 0 ? void 0 : i.name) ||
      e.name;
  ...
  if (
    e.name == 'Comment' ||
    e.name == 'StringLiteral' ||
    e.name == 'NumberLiteral'
  )
    return null;
  let Q = O.matchBefore(/[\w\u00c0-\uffff]+$/);
  return Q
    ? { type: 'expr', from: Q.from }
    : O.explicit
      ? { type: 'expr' }
      : null;
}
      ...
      props: [
        Z({
          'TagName raw endraw filter endfilter as trans pluralize endtrans with endwith autoescape endautoescape':
            a.keyword,
          'required scoped recursive with without context ignore missing':
            a.modifier,
          self: a.self,
          'loop super': a.standard(a.variableName),
          'if elif else endif for endfor call endcall': a.controlKeyword,
          'block endblock set endset macro endmacro import from include':
            a.definitionKeyword,
        ...
        h.add({
          'Statement Comment'(O) {
            ...
            return !i || (i.name != 'Tag' && i.name != '{#')
              ...
              : {
                  from: i.to,
                  to: t.name == 'EndTag' || t.name == '#}' ? t.from : O.to,
                };
          },
        }),
      ],
