...
function $(O, e, i) {
  ((this.parent = O),
    ...
    (this.flags = i),
    (this.hash =
      (O ? (O.hash + O.hash) << 8 : 0) + e + (e << 4) + i + (i << 6)));
}
...
const V = new Map(
  ...
  se = new H({
    start: oe,
    reduce(O, e, i, a) {
      return (O.flags & m && re.has(e)) || ((e == vO || e == Y) && O.flags & J)
        ? O.parent
        : O;
    },
    shift(O, e, i, a) {
...
function Se(O, e) {
  if (e == Oe)
    ...
    for (
      O.advance();
      O.next >= 0 && O.next != U && O.next != F && O.next != C && O.next != p;
    )
      O.advance();
    O.next == U && O.advance();
  }
...
const Pe = {
  ...
  ImportStatement(O, e) {
    ...
    let { node: r } = O,
      Q =
        ((i = r.firstChild) === null || i === void 0 ? void 0 : i.name) ==
        'from';
    for (let t = r.getChild('import'); t; t = t.nextSibling)
      t.name == 'VariableName' &&
        ((a = t.nextSibling) === null || a === void 0 ? void 0 : a.name) !=
          'as' &&
...
function me(O) {
  ...
  let i =
    e.name == 'VariableName' ||
    (e.to - e.from < 20 && w.test(O.state.sliceDoc(e.from, e.to)));
    ...
    props: [
      eO.add({
        ...
        MatchBody: O => {
          var e;
          let i = z(O);
          return (e = W(O, i || O.node)) !== null && e !== void 0
            ? e
            : O.continue();
        },
        IfStatement: O =>
          /^\s*(else:|elif )/.test(O.textAfter) ? O.baseIndent : O.continue(),
